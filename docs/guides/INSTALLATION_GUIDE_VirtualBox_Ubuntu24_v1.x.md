...
4. Executes **`database/db-init.sql`** which creates **all 15 tables, constraints, indexes**.
5. Loads full sample data (vendors, NACHA settings, payment batches, etc.) via  
...
# 2. Consolidated schema  (creates every table & constraint)
sudo -u postgres psql -d fund_accounting_db -f database/db-init.sql

# 3. Sample data (optional / idempotent)
sudo -u postgres psql -d fund_accounting_db -f database/insert-complete-nacha-data.sql
```
...
node server-modular.js        # or simply `npm start`
```
...
npm start        # backend only (server-modular.js)
```
...
| DB inspection | `psql -d fund_accounting_db -c '\dt'` | 15 tables incl. `payment_batches`, `nacha_files` |
...
| Empty dropdowns (batch modal) | Run `database/insert-complete-nacha-data.sql` & restart API |
